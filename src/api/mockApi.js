...
export function searchEvents(query = '') {
  return axios.get('http://localhost:8000/api/Events?q='+ query)
    .then(response => {
      store.dispatch(getEventsSuccess(response.data))
      return response
    })
...
export function deleteEvent(eventId) {
  return axios.delete('http://localhost:8000/api/Events/' + eventId)
    .then(response => {
...
export function getProfile(eventId) {
  ...
  return axios.get('http://localhost:8000/api/Events/' + eventId)
    .then(response => {
      ...
      profile.degree = event.degree ? event.degree : '00:00'

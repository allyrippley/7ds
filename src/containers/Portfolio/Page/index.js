...
const PortfolioPage = (props) => {
  window.console.log('PortfolioPageProps: ', props)
  const index = props.params.id - 1
        ...
        <div style={{display: 'flex', flex: 1, alignItems: 'center', justifyContent: 'space-around', height: '100%', width: '100%'}}>
          {item.id > 1 &&
            <div style={{flex: 0.1, fontSize: '3em', margin: 15, textAlign: 'left'}}>
              <Link to={`/portfolio/page/${Number.parseInt(props.params.id)-1}`}><Fa name="angle-left" size="3x" /></Link>
            </div>
          }
          <img style={{flex: 0.4, height: 'auto', maxHeight: '80%', width: 'auto', maxWidth: '50%', margin: 20}} src={item.src} />
          ...
          {item.id < itemLength &&
            <div style={{flex: 0.1, fontSize: '3em', margin: 15, textAlign: 'right'}}>
              <Link to={`/portfolio/page/${Number.parseInt(props.params.id)+1}`}><Fa name="angle-right" size="3x" /></Link>
            </div>
          }
        </div>
      </div>
